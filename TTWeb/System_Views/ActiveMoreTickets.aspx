<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ActiveMoreTickets.aspx.cs" Inherits="TTWeb.ActiveMoreTickets" %>
...
<body style="font-size: 8pt">
    ...
    function OnGetSelectedFieldValues(selectedValues) {
        for (var i = 0; i < 36; i++) {
            var editor = ASPxGridViewActiveMoreTickets.GetEditor(i + 1);
            if (editor != null) {
        ...
        <dx:ASPxRoundPanel ID="ASPxRoundPanel1" runat="server" ClientIDMode="AutoID" 
            ...
            <HeaderTemplate>
                <table style="width: 100%">
                    <tr>
                        <td width="100%" valign="middle" align="left">Active Tickets
                        </td>
                    ...
                    <tr>
                        <td style="padding-right: 0px; border-right-width: 0px; margin-right: 0px; padding-left: 0px; width: 0%;">
                            <dx:ASPxMenu ID="ASPxMenuAttachment" runat="server" 
                                Theme="Metropolis" ClientInstanceName="menuAttachment" 
                                OnLoad="ASPxMenuAttachment_Load" Width="200px">
                                <ClientSideEvents ItemClick="function(s, e) {

	 if(e.item.name == &quot;CreateSRI&quot;) 
	{
        popupsri1.Show();
        CallbackSRI.PerformCallback(ASPxGridViewActiveMoreTickets.GetRowKey(ASPxGridViewActiveMoreTickets.GetFocusedRowIndex()));
    }

	 if(e.item.name == &quot;AddSuppl&quot;) 
	{          
        var cmbType = typecombo.GetValue();
		if(!cmbType)
		{
			alert('Please select ticket type first!');
		}
		else
		{
...
" />
                        ...
                        <td style="padding-right: 0px; border-right-width: 0px; margin-right: 0px; padding-left: 5px; width: 100%; background-image: ~/Images/back2.png; background-repeat: repeat;" 
                            width="100%">
                            <dx:ASPxComboBox ID="TypeCombo" runat="server" Font-Size="8pt" 
                                Theme="Metropolis" ClientInstanceName="typecombo" 
                                OnInit="ASPxComboBoxType_Init" 
                                OnSelectedIndexChanged="TypeCombo_SelectedIndexChanged">
                            </dx:ASPxComboBox>
                            </td>
                    </tr>
                </table>
    ...
    ClientInstanceName="ASPxGridViewActiveMoreTickets" 
    CssFilePath="~/App_Themes/PlasticBlue/{0}/styles.css" CssPostfix="PlasticBlue" 
    KeyFieldName="MORE_TICKET_ID" AutoGenerateColumns="False" Width="100%" 
    ClientIDMode="AutoID" ID="ASPxGridViewActiveMoreTickets" 
    EnableViewState="False" 
        Theme="Metropolis" style="font-size: 8pt" 
        OnHtmlRowPrepared="ASPxGridViewActiveMoreTickets_HtmlRowPrepared" 
        OnRowInserting="ASPxGridViewActiveMoreTickets_RowInserting" 
        OnRowDeleting="ASPxGridViewActiveMoreTickets_RowDeleting" 
        OnRowUpdating="ASPxGridViewActiveMoreTickets_RowUpdating" 
        OnLoad="ASPxGridViewActiveMoreTickets_Load" 
        
        
        OnCellEditorInitialize="ASPxGridViewActiveMoreTickets_CellEditorInitialize1">
...
<SettingsCookies Enabled="True" Version="11" StoreColumnsVisiblePosition="False"></SettingsCookies>
...
if (e.result.split(';')[2] == &quot;no&quot;)
{
        ...
        ASPxGridViewActiveMoreTickets.Refresh();
} 
else
{
		popupsri1.Hide();
    ...
    <dx:ASPxGridViewExporter ID="ASPxGridViewExporter2" runat="server" 
        GridViewID="ASPxGridViewActiveMoreTickets" Landscape="True" PaperKind="A4">
        <Styles>
            <Default Font-Size="8pt">
            </Default>
        </Styles>
        <PageHeader Left="Active Tickets " 
            Right="[Page # of Pages #]">
        </PageHeader>
